package com.distribike.features.subfeatures.form.main.forms.coolingform
...
@AndroidEntryPoint
class CoolingFormActivity : ComponentActivity() {

    companion object {
        fun newInstance(context: Context) = Intent(context, CoolingFormActivity::class.java)
    }

    private val viewModel: CoolingFormViewModel by viewModels()
            ...
            Column {
                ...
                if (data.value.sections.isNotEmpty()) {
                    Text(
                        ...
                        text = data.value.sections[6].title,
                        style = TextStyle(
                    ...
                    val steps = List(
                        data.value.sections[6].tasks.size
                    ) { index ->
                        Step(
                            title = data.value.sections[6].tasks[index].title
                        ) {
                            Row {
                                WorkerLottie(
                                    modifier = Modifier
