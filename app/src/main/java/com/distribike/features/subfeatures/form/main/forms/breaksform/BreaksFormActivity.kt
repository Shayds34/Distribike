package com.distribike.features.subfeatures.form.main.forms.breaksform
...
@AndroidEntryPoint
class BreaksFormActivity : ComponentActivity() {

    companion object {
        fun newInstance(context: Context) = Intent(context, BreaksFormActivity::class.java)
    }

    private val viewModel: BreaksFormViewModel by viewModels()
            ...
            Column {
                ...
                if (data.value.sections.isNotEmpty()) {
                    Text(
                        ...
                        text = data.value.sections[3].title,
                        style = TextStyle(
                    ...
                    val steps = List(
                        data.value.sections[3].tasks.size
                    ) { index ->
                        Step(
                            title = data.value.sections[3].tasks[index].title
                        ) {
                            Row {
                                WorkerLottie(
                                    modifier = Modifier
